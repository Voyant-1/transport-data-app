...
function TransportData() {
    ...
    try {
      ...
      if (data.length > 0) {
        setTransportData(prevData => {
          const combinedData = isLoadMore ? [...prevData, ...data] : data;
        
          // Remove duplicates based on 'dot_number'
          const uniqueData = combinedData.reduce((acc, current) => {
            const found = acc.find(item => item.dot_number === current.dot_number);
            if (!found) {
              acc.push(current);
            }
            return acc;
        ...
        // Only increment cumulative count if loading more data
        if (isLoadMore) {
          setCumulativeResultsCount(prevCount => prevCount + data.length);
        } else {
          setCumulativeResultsCount(data.length); // Reset count for a new search
        }
      } else {
        setNoResults(true);
        setCumulativeResultsCount(0); // Reset count if no results are found
      }
  ...
  const handleSearchChange = (event) => {
    ...
    setCumulativeResultsCount(0); // Reset count
  };
  ...
  const handlePowerUnitsFilterChange = (value) => {
    setPowerUnitsValue(value);
    setTransportData([]);
    setTotalRowsFetched(0);
    setCumulativeResultsCount(0); // Reset count
  };
  
  const handlePowerUnitsComparisonChange = (value) => {
    setPowerUnitsComparison(value);
    setTransportData([]);
    setTotalRowsFetched(0);
    setCumulativeResultsCount(0); // Reset count
  };
  ...
  {loading ? (
    ...
    <p>
      {cumulativeResultsCount > 0
        ...
        {/* Load More Button */}
        {transportData.length > 0 && !noResults && totalRowsFetched < cumulativeResultsCount && (
  <button 
    onClick={handleLoadMore} 
    disabled={totalRowsFetched >= cumulativeResultsCount || loading} 
